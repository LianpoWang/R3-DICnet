...
def main():
    ...
    img1_path = "essaib000.BMP"
    img2_path = "essaib400.BMP"
    ...
    mask = cv.imread("1.png")  # (1920, 2560)
    mask  = np.where(mask  == 255, 1, 0)  # 想要的是1，不想要的是0
    ...
    for (img1_file, img2_file) in tqdm(img_pairs):
        ...
        print(disp_y.shape)
        sc = plt.imshow(disp_y*mask[:,:,1], cmap=plt.cm.jet)
        plt.colorbar()  # 显示色度条
        plt.show()
